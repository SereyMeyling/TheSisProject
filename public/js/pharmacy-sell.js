$(function () {
    ...
    function loadMedicineSelect($select) {
        $.get(routes.sellSearch).done((list) =>
            $select.html(medicineOptionsHtml(list)),
        );
    }
    ...
    function recalcSellTotal() {
        ...
        $("#sellItemsBody tr").each(function () {
            const price =
                parseFloat(
                    $(this)
                        .find(".sell-medicine option:selected")
                        .data("price"),
                ) || 0;
    ...
    function loadHistory() {
        $.get(routes.sellHistory).done((res) => {
            ...
            `,
                    ...
                    .join("") ||
                '<tr><td colspan="4" class="text-center text-muted">មិនទាន់មានការលក់</td></tr>';
    ...
    $("#sellForm").on("submit", function (e) {
        ...
        $.ajax({
            ...
            .fail((xhr) => {
                ...
                showToast("លក់មិនបានសម្រេច", "error");
            });
    });
    // ---- Patient dropdown ----
