$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                history.replaceState(
                    null,
                    "",
                    window.billingConfig.indexUrl + "?" + qs,
                );
            },
            error: function () {
    ...
    $(document).on(
        "click",
        "#billingTableContainer .pagination a",
        function (e) {
            e.preventDefault();
    ...
    function defaultCreateItemRowHtml(idx) {
        return itemRowHtml(idx, {
            item_type: "",
            desc: "ថ្លៃពិគ្រោះជំងឺទូទៅ",
            qty: 1,
            price: "15.00",
        });
    }
        ...
        $.ajax({
            ...
            success: function (res) {
                $btn.prop("disabled", false).html(
                    '<i class="fas fa-save mr-1"></i> រក្សាទុកវិក្កយបត្រ',
                );
                $("#modalCreateInvoice").modal("hide");
                loadInvoices(1);
                showBillingToast(
                    res.message || "បានបង្កើតវិក្កយបត្រដោយជោគជ័យ!",
                );
            },
            error: function (xhr) {
                $btn.prop("disabled", false).html(
                    ...
                    $alert
                        ...
                        .text(
                            xhr.responseJSON?.message ||
                                "មានបញ្ហាក្នុងការបង្កើតវិក្កយបត្រ",
                        );
                }
            },
        });
    ...
    $(document).on("click", ".btn-pay-now", function () {
        ...
        const payUrl =
            (window.billingConfig.baseUrl || "{{ url('billing') }}") +
            "/" +
            id +
            "/pay";
    ...
    function generateInvoiceKHQR() {
        ...
        if (!invoiceId || !balance || balance <= 0) {
            $("#khqrQrWrap").html(
                '<span class="text-danger small">ទិន្នន័យវិក្កយបត្រមិនត្រឹមត្រូវ</span>',
            );
            return;
        }
        ...
        $("#khqrStatusMessage").html(
            '<span class="text-muted">កំពុងបង្កើត QR...</span>',
        );

        $.post(
            window.billingConfig.generateKhqrUrl,
            {
            ...
            function (res) {
                if (!res.success) {
                    $("#khqrQrWrap").html(
                        '<span class="text-danger small">' +
                            (res.message || "មិនអាចបង្កើត QR បានទេ") +
                            "</span>",
                    );
                    return;
                }
                ...
                if (res.mode === "manual") {
                    ...
                    $("#khqrQrWrap").html(
                        `<img src="${res.qr_image_url}" alt="KHQR" style="max-width: 200px;">`,
                    );
                    ...
                    const src =
                        String(res.qr).startsWith("data:") ||
                        String(res.qr).startsWith("http")
                            ? res.qr
                            : res.qr; // qr string itself isn't an image — see note below
                    $("#khqrQrWrap").html(
                        `<img src="data:image/svg+xml;base64,${res.qr}" alt="KHQR" style="max-width: 200px;">`,
                    );
            ...
            $("#khqrQrWrap").html(
                '<span class="text-danger small">មិនអាចបង្កើត QR បានទេ</span>',
            );
        });
    }
    ...
    $(document).on("click", "#btnConfirmManualPaid", function () {
        const $btn = $(this);
        $btn.prop("disabled", true).html(
            '<i class="fas fa-spinner fa-spin mr-1"></i> កំពុងបញ្ជាក់...',
        );

        $.post(
            `${window.billingConfig.baseUrl}/${$("#pay_invoice_id").val()}/pay`,
            {
                _token: window.billingConfig.csrfToken,
            ...
            $btn.prop("disabled", false).html(
                '<i class="fas fa-check-circle mr-1"></i> បញ្ជាក់ថាបានទទួលប្រាក់',
            );
    ...
    function pollPaymentStatus(md5) {
        ...
        khqrPollTimer = setInterval(function () {
            ...
            if (khqrPollAttempts > 60) {
                clearInterval(khqrPollTimer);
                $("#khqrStatusMessage").html(
                    '<span class="text-danger">QR បានផុតកំណត់ សូមព្យាយាមម្តងទៀត</span>',
                );
                return;
            }

            $.get(
                `${window.billingConfig.checkKhqrStatusUrlBase}/${md5}`,
                function (res) {
                    if (res.paid) {
                        clearInterval(khqrPollTimer);
                        $("#khqrStatusMessage").html(
                            '<span class="text-success"><i class="fas fa-check-circle mr-1"></i>ការទូទាត់បានជោគជ័យ</span>',
                        );
                        showBillingToast("ការទូទាត់វិក្កយបត្របានជោគជ័យ!");
    ...
    $(document).on("submit", "#formPayInvoice, #payInvoiceForm", function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                $btn.prop("disabled", false).html(
                    '<i class="fas fa-check-circle mr-1"></i> បញ្ជាក់ការទូទាត់',
                );
                $("#modalPayInvoice").modal("hide");
                loadInvoices(1);
                showBillingToast(
                    res.message || "បានបញ្ចប់ការទូទាត់វិក្កយបត្រដោយជោគជ័យ!",
                );
            },
            error: function (xhr) {
                $btn.prop("disabled", false).html(
                ...
                alert(
                    xhr.responseJSON?.message || "មានបញ្ហាក្នុងការទូទាត់ប្រាក់",
                );
            },
        });
    });
    ...
    $(document).on("click", ".btn-view-receipt", function () {
        ...
        $.ajax({
            ...
            error: function () {
                $("#modalReceiptBody").html(
                    '<div class="alert alert-danger mb-0">ពុំអាចទាញយកទិន្នន័យវិក្កយបត្របានឡើយ។</div>',
                );
            },
        });
    });
    ...
    function getInvoiceStatusBadge(status) {
        const statusMap = {
            ...
            partial: {
                text: "បង់ខ្លះ(Partial)",
                class: "badge-status-partial",
            },
            unpaid: {
                text: "មិនទាន់បង់(Unpaid)",
                class: "badge-status-unpaid",
            },
            cancelled: {
                text: "បានលុបចោល(Cancelled)",
                class: "badge-status-cancelled",
            },
        };
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                `);
                    ...
                    $("#viewAdmissionWrap, #viewRoomWrap").removeClass(
                        "d-none",
                    );
                } else {
                    $("#viewVisitType").html(`
                ...
                $("#viewCancelledBanner").toggleClass(
                    "d-none",
    ...
    // =====================================================
    // Open invoice detail from notification
    // URL example:
    // /billing?view=5
    // =====================================================
    ...
    $(document).ready(function () {
        ...
        if (invoiceId) {
            ...
            // Remove ?view=5 from browser URL
    ...
    $("#btnAddEditInvoiceItem").on("click", function () {
        $("#editInvoiceItemsTbody").append(
            editItemRowHtml(null, editItemIndex),
        );
        editItemIndex++;
    ...
    $(document).on(
        "click",
        "#editInvoiceItemsTbody .btn-remove-item",
        function () {
    ...
    $(document).on(
        "input",
        "#editInvoiceItemsTbody .item-qty, #editInvoiceItemsTbody .item-price",
        function () {
            const $row = $(this).closest("tr");
            const qty = parseFloat($row.find(".item-qty").val()) || 0;
            const price = parseFloat($row.find(".item-price").val()) || 0;
    ...
    $(document).on("click", ".btn-edit-invoice", function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (!res.success) {
                    showBillingToast(
                        res.message || "Invoice can no longer be edited.",
                    );
                    loadInvoices(1);
                    return;
                }
                ...
                (inv.items && inv.items.length ? inv.items : [null]).forEach(
                    function (item) {
                        $("#editInvoiceItemsTbody").append(
                            editItemRowHtml(item, editItemIndex),
                        );
                        editItemIndex++;
                    },
                );
                ...
                $("#editInvoiceForm").attr(
                    "action",
                    window.billingConfig.baseUrl + "/" + id,
                );
    ...
    $("#editInvoiceForm").on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                loadInvoices(1);
                showBillingToast(
                    res.message || "បានកែប្រែវិក្កយបត្រដោយជោគជ័យ!",
                );
            },
            error: function (xhr) {
                $btn.prop("disabled", false).html(
    ...
    $(document).on("click", ".btn-cancel-invoice", function () {
        ...
        $("#cancelInvoiceForm").attr(
            "action",
            window.billingConfig.baseUrl + "/" + id + "/cancel",
        );
